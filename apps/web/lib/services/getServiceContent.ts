...
/**
 * Merge database content with static service data
 * DB content overrides static where available
 */
...
/**
 * Get service content for public display
 * This function is called at BUILD time and on REVALIDATION
 * 
 * Priority:
 * 1. Published content from database
 * 2. Static content from data/services
 * 
 * @param slug - Service slug
 * @returns Service data merged with any published DB content
 */
export async function getServiceContent(slug: string): Promise<ServiceContentResult | null> {
    // Get static service as base (required for fallback)
    ...
    try {
        ...
        if (docs.documents.length > 0) {
            ...
            console.log(`[ISR] Service "${slug}" loaded from database (v${dbContent.version})`);

            return {
                service: mergedService,
                source: 'database',
                version: dbContent.version,
                publishedAt: dbContent.publishedAt || undefined,
            };
        }
    } catch (dbError) {
        // If database lookup fails, fall back to static content
        console.warn(`[ISR] Database lookup failed for "${slug}", using static:`, dbError);
    }

    // Return static service as fallback
    console.log(`[ISR] Service "${slug}" loaded from static data`);
