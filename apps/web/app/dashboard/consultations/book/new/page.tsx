...
import {
    ...
    ArrowLeft, Check, Video, Globe, FileText
} from 'lucide-react';
import { cn } from '@/lib/utils';
import { toast } from 'sonner';
import { CASE_TYPE_LABELS, CaseType, BusinessType } from '@lawethic/appwrite/types';
...
export default function NewBookingPage() {
    ...
    const [consultationData, setConsultationData] = useState<PendingConsultation | null>(null);
    ...
    useEffect(() => {
        // Load consultation data from sessionStorage
        const storedData = sessionStorage.getItem('pendingConsultation');
        if (!storedData) {
            toast.error('Please fill out the consultation form first');
            router.push('/dashboard/consultations/new');
            return;
        }

        try {
            const parsed = JSON.parse(storedData) as PendingConsultation;
            setConsultationData(parsed);
        } catch (error) {
    ...
    const handleBookSlot = async () => {
        if (!selectedSlot || !consultationData) return;

        try {
            setBooking(true);

            // Create case AND book slot in one API call
            const response = await fetch('/api/consultations/book-with-case', {
                method: 'POST',
            ...
            if (data.success) {
                ...
                router.push(`/dashboard/consultations/book/${data.caseId}/success?${params.toString()}`);
            } else {
        ...
        return (
            <div className="p-6 flex items-center justify-center min-h-[400px]">
                <div className="text-center">
                    <h1 className="text-xl font-semibold text-neutral-900">No consultation data found</h1>
                ...
                <div className="lg:col-span-1">
                    <div className="bg-white rounded-2xl border border-neutral-200 p-6 sticky top-6">
                        ...
                        <div className="mt-6 pt-4 border-t border-neutral-100">
                            <p className="text-xs text-neutral-500 mb-1">Your Consultation</p>
                            <p className="font-medium text-neutral-900">{consultationData.title}</p>
                            {consultationData.caseType && (
                                <p className="text-sm text-neutral-500 mt-1">
                                    {CASE_TYPE_LABELS[consultationData.caseType]}
                                </p>
                            )}
                            {consultationData.attachments.length > 0 && (
                                <div className="flex items-center gap-2 mt-2 text-sm text-neutral-500">
                                    <FileText className="w-4 h-4" />
                                    <span>{consultationData.attachments.length} attachment(s)</span>
                                </div>
                            )}
                        </div>
                    </div>
